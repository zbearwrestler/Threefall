...
public class CameraPersonJoystickView : MonoBehaviour
{
    ...
    private void Update()
    {
        currX += Input.GetAxis("Mouse X_Joystick");
        currY += Input.GetAxis("Mouse Y_Joystick");
        newX += Input.GetAxis("Mouse X_Joystick");
        currY = Mathf.Clamp(currY, Y_ANGLE_MIN, Y_ANGLE_MAX);
        newY += Input.GetAxis("Mouse Y_Joystick");
    }
